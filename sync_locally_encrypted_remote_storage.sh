...
if [ ! -e /tmp/${extHOST}_hd/A/${extImgName}.img ]; then
	...
	ssh ${extUser}@${extHOST} "dd if=/dev/urandom of=${extPath}/${extImgName}.img bs=1M count=${imageSize}"
	...
	cryptsetup -c aes-xts-plain -s 512 luksFormat /tmp/${extHOST}_hd/A/${extImgName}.img key_${extHOST}_${extImgName}.txt
	shred key_${extHOST}_${extImgName}.txt
	rm key_${extHOST}_${extImgName}.txt
fi

#sudo mkdir -p /tmp/${extHOST}_hd/B
#sudo chmod 700 /tmp/${extHOST}_hd/B
...
if [ ! -d /tmp/${extHOST}_hd/C/${extImgName}.git ]; then
	mkdir -p /tmp/${extHOST}_hd/C/${extImgName}.git
